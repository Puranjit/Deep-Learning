# -*- coding: utf-8 -*-
"""
Created on Fri Apr  1 06:28:40 2022
...
def jaccard_coef(y_true, y_pred):
    y_true_f = K.flatten(y_true) # true 
    y_pred_f = K.flatten(y_pred) # predicted
    intersection = K.sum(y_true_f * y_pred_f) # Intersection
    return (intersection + 1.0) / (K.sum(y_true_f) + K.sum(y_pred_f) - intersection + 1.0) # Intersection over union
...
################################################################
def simple_unet_model_with_jaccard(IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS):
#Build the model
